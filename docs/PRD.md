...
- **Next.js 16.2** (App Router, Turbopack) - 현재 저장소 기준 프레임워크
- **React 19.2** - UI 라이브러리
...
- **shadcn/ui (`@base-ui/react` 기반)** - 기존 저장소의 `components/ui/*` 프리미티브 재사용
- **Lucide React** - 아이콘 라이브러리

### 📝 폼 & 검증

- **React Hook Form 7.x** - 로그인 폼 상태 관리
- **Zod 4** - 스키마 검증 (기존 `lib/validations.ts` 패턴 확장)

### 🗄️ 백엔드 & 데이터베이스

- **Supabase** - 인증(Auth) + PostgreSQL DB (견적서/품목 캐시 저장)
- **Notion API (`@notionhq/client`)** - 노션 데이터베이스에서 견적서 원본 데이터 조회

### 📄 PDF 생성

- **@react-pdf/renderer** - 클라이언트 사이드 PDF 문서 생성/다운로드

...
5. **UI/UX 개발**
   - shadcn/ui (`components.json`에 설정된 스타일) 컴포넌트 활용
...
### Next.js 핵심 규칙 (설치된 버전 기준으로 검증할 것)

#### 1. async request APIs 처리
```typescript
// 🔄 params와 searchParams는 Promise (버전별 정확한 시그니처는 node_modules/next/dist/docs로 확인)
...
작업 완료 전 반드시 확인 (이 프로젝트의 `package.json`에는 `dev`/`build`/`start`/`lint`만 정의되어 있고 별도의 `typecheck`/`format`/`check-all` 스크립트는 없음. 타입 체크는 `npx tsc --noEmit`으로 수행):
```bash
npm run lint        # ESLint 검사
npx tsc --noEmit    # TypeScript 타입 체크
npm run build       # 프로덕션 빌드 성공 확인 (타입 에러도 함께 검출)
```
...
4. **구현**
   ...
   - 설치된 Next.js 버전의 async request APIs 정확히 사용
...
5. **Turbopack 최적화**
   ```typescript
   // next.config.ts — 이 프로젝트는 아이콘에 lucide-react, 프리미티브에 통합 radix-ui 패키지를 사용
   experimental: {
...
1. **문서 검색**:
   ...
   - `mcp__context7__query-docs`: Next.js/React 최신 문서
...
1. **검증**:
   - `mcp__supabase__get_advisors`: 최종 보안/성능 체크
   - `npm run lint` + `npx tsc --noEmit` + `npm run build`: 코드 품질 검사
...
2. **성능 최적화**: Next.js 새 기능(Streaming, after API 등) 적극 활용 (설치된 버전에서 지원 여부 확인)

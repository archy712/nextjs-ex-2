...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직을 다루는 모든 Task는 "## 테스트 체크리스트" 섹션을 반드시 포함하고, 정상 케이스뿐 아니라 실패·엣지 케이스까지 꼼꼼히 시나리오로 작성**
- **구현만으로는 Task가 완료된 것이 아니며, Playwright MCP로 테스트를 수행해 통과를 확인해야 비로소 완료로 간주** (테스트 생략 불가)
- 테스트가 실패하면 다음 Task로 넘어가지 않고 해당 Task로 돌아가 원인을 수정한 뒤 재검증
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **API 연동 및 비즈니스 로직 구현은 "구현 → 테스트"가 하나의 완료 기준이며, 테스트를 생략한 구현은 미완료로 간주**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- **구현 완료 직후 Playwright MCP로 E2E 테스트 실행 필수** — 정상/실패/엣지 케이스를 모두 검증하고, 필요 시 `browser_network_requests`·`browser_console_messages`로 실제 요청·응답·에러를 직접 확인
- 테스트를 통과한 경우에만 다음 단계로 진행, 미통과 시 원인을 수정한 뒤 재테스트
...
5. **테스트 필수화**: API 연동/비즈니스 로직을 포함하는 Task는 예외 없이 "## 테스트 체크리스트" 섹션을 함께 작성하고, Playwright MCP로 정상·실패·엣지 케이스를 모두 검증하기 전까지 완료 처리 금지
...
### 🧪 테스트 작성 원칙 (API 연동 · 비즈니스 로직 Task 전용)

API 연동 및 비즈니스 로직을 다루는 모든 Task는 다음 원칙을 따라 "## 테스트 체크리스트" 섹션을 작성합니다. 구현이 끝났다고 Task를 완료로 표시하지 말고, 반드시 아래 테스트를 Playwright MCP로 수행해 통과를 확인한 뒤에만 완료로 표시합니다.

#### **케이스 커버리지 (세 종류 모두 포함)**

- **정상 케이스**: 예상된 입력·정상 인증·정상 네트워크 상태에서 의도대로 동작하는지
- **실패 케이스**: 잘못된 입력, 인증/권한 실패, 존재하지 않는 리소스, 네트워크·API 오류(4xx/5xx) 시 에러 처리가 올바른지
- **엣지 케이스**: 빈 값/0/최대값 등 경계값, 대량 데이터, 동시 요청, 느린 응답(타임아웃) 등

#### **Playwright MCP 활용**

- `browser_navigate`로 실제 플로우 재현, `browser_snapshot`으로 접근성 트리 기반 UI 상태 확인
- `browser_network_requests`로 실제 요청/응답(상태 코드, 페이로드)이 기대와 일치하는지 확인
- `browser_console_messages`로 예기치 않은 에러·경고(예: hydration mismatch)가 없는지 확인
- `browser_take_screenshot`으로 시각적 회귀가 필요한 경우 스크린샷 확보
- 폼/입력이 있는 경우 `browser_fill_form`, `browser_click` 등으로 사용자 행동 재현

#### **완료 판정 규칙**

- 테스트 시나리오를 하나라도 실행하지 않고 완료(✅) 표시 금지
- 테스트가 실패하면 해당 Task를 완료 처리하지 않고, 원인을 수정한 뒤 재검증 — 다음 Task로 넘어가지 않음
- Task 완료 표시 시 어떤 시나리오가 통과했는지 요약을 함께 기록
...
- [ ] 테스트 시나리오가 정상·실패·엣지 케이스를 모두 포함하는가? (정상 동작만 확인하는 테스트는 불충분)
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] "구현 완료 = 테스트 통과 확인"이 Task 완료 조건으로 명시되어, 테스트 없이 완료 표시할 수 없도록 되어 있는가?

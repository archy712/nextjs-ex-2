...
- 상세한 기술 스택과 용도별 분류
- **반드시 최신 버전 명시**: 아래 예시(Next.js 16, React 19 등)는 작성 시점 기준이므로, PRD 작성 시점에 실제 최신 안정 버전을 재확인하여 명시 (숫자를 그대로 베끼지 말 것)
...
- **Next.js 16**(예시, 작성 시점 최신 안정 버전으로 재확인) (App Router) - React 풀스택 프레임워크
- **TypeScript 5.9+**(예시, 작성 시점 최신 버전으로 재확인) - 타입 안전성 보장
- **React 19**(예시, 작성 시점 최신 버전으로 재확인) - UI 라이브러리 (최신 동시성 기능)
...
6. **최신 기술**: **반드시 현재 최신 버전** 명시 (작성 시점에 실제 최신 버전을 확인할 것 — 하드코딩된 예시 숫자를 그대로 쓰지 말 것)

## 🔧 기술 스택 선택 원칙

- **최신 버전 필수**: Next.js, React, TailwindCSS v4 등 작성 시점의 실제 최신 버전 사용
- **Next.js 최신 메이저 버전 기반**: 최신 App Router, 향상된 성능, 최신 React 지원
...
- Next.js (작성 시점 최신 안정 버전 확인 후 명시 — 예: 이 문서 작성 시점 기준 Next.js 16)
- React (작성 시점 최신 버전 확인 후 명시 — 예: 이 문서 작성 시점 기준 React 19)
- TailwindCSS v4 (설정파일 없는 새로운 방식)
- 각 기술의 최신 버전을 추측하지 말고 확인 후 명시
